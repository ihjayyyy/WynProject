...
export default function PRForm() {
      ...
      const initialMode = searchParams.get('mode') || (initialformId ? 'view' : 'edit');
   ...
   const onFormChange = (fieldname,value, data)=>{
    ...
    const [itemFields,setItemFields] = useState(ItemsFields(materials,formData)) 
...
const formTitle = useMemo(() => {
    const title =  formData &&  formData.status ? formData.orderNumber : 'New Purchase Request';
...
const submitForApproval = async()=>{
  ...
  const ApprovalButton = () =>{
    ...
    const OrderButton = () =>{
    return isAllowed(PageName, 'ww') && formId && formData.status && formData.status.toLowerCase() === "approved"  ?
    <div  className={EntityStyle.buttonsContainer}>
    ...
    const ArchiveButton = () =>{
    return isAllowed(PageName, 'w') && mode === 'view'  && formId && formData.status && (formData.status.toLowerCase() === "ordered" 
      || formData.status.toLowerCase() === "approved" || formData.status.toLowerCase() === "rejected" || formData.status.toLowerCase() === "cancelled") ?
    <div  className={EntityStyle.buttonsContainer}>
