...
export const TableColumns = [
...

  {
    ...
    width: '120px',
    render: (it) => {
      return (it.rackCode || it.rackName || it.rack?.code || '');
    }
  },
...

  {
    ...
    align: 'right',
    width: '80px',
    render: (it) => Number(it.orderQuantity || 0).toFixed(0)
  },

  {
    ...
    align: 'right',
    width: '80px',
    render: (it) => Number(it.previousBalance || 0).toFixed(0)
  },
...

  {
    ...
    align: 'right',
    width: '80px',
    render: (it) => Number(it.remainingBalance || 0).toFixed(0)
  },
...
export const ItemsFields = (materials, dr) => ([
...

  {
    ...
    onChange: async (item, updateField, fields) => {
      ...
      try {
        ...
        const hasCurrentRack = rackOptions.some(
          (r) => Number(r.value) === currentRackId
        );

        const defaultRack =
          rackOptions.find((r) => r.isDefault) || null;
...

  {
    ...
    onChange: (item, updateField, fields) => {
      ...
      const selectedRack =
        rackField && Array.isArray(rackField.options)
          ? rackField.options.find(
              (r) => String(r.value) === String(item.value)
            )
          : null;
      ...
      updateField("rackCode", selectedRack.code || "");
      updateField("rackName", selectedRack.rackName || "");
    }
  },
...

  {
    ...
    onChange: (item, updateField, fields) => {
      ...
      const previousBalanceField =
        fields.find(a => a.name === 'previousBalance');

      const previousBalance = Number(
        previousBalanceField?.value || 0
      );
      ...
      updateField("remainingBalance", Number(bal));
    },
  },
