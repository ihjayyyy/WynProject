...
function RightPanel({
  ...
  // Local state for purchase type filter (if not controlled by parent)
  const [purchaseType, setPurchaseType] = useState(filter?.purchaseType || '');
        ...
        <div className={styles.panelContent}>
          <div className={styles.filterSection}>
            <label className={styles.filterLabel}>
              <span className={styles.filterLabelText}>Purchase Type</span>
              <Select
                id="purchaseTypeFilter"
                value={purchaseType}
                onChange={(e) => {
                  setPurchaseType(e.target.value);
                  if (onFilterChange) {
                    onFilterChange({ ...filter, purchaseType: e.target.value });
                  }
                }}
                options={[
                  { value: '', label: 'All' },
