...
export default function MaterialTransferLanding() {
  ...
  const stats = useMemo(() => {
    ...
    const warehouseToProject = list.filter(
      (d) =>
        d.transferFromType === 'Warehouse' && d.transferToType === 'Projects'
    ).length;
    const projectToWarehouse = list.filter(
      (d) =>
        d.transferFromType === 'Projects' && d.transferToType === 'Warehouse'
    ).length;

    return [
      { key: 'total', label: 'Total Transfers', number: total, change: `${total} records`, isPositive: true },
      { key: 'items', label: 'Total Items', number: totalItems, change: `${totalItems} items`, isPositive: true },
