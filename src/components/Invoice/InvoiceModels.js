...
export const FormFields = (suppliers, orders, onFieldhanged) => [
  ...
  { name: 'spacer-2', type: 'spacer', span: 'span2' },
  {
    ...
    label: 'PO Number',
    type: 'select',
    options: orders.map((s) => ({ label: s.orderNumber, value: s.id })),
    searchable: true,
    span: 'span2',
    onChange: (val, values, setValues) => {
  ...
  { name: 'spacer-10', type: 'spacer', span: 'span2' },
  { name: 'dueDate', label: 'Due Date', type: 'date', span: 'span2' },
  { name: 'code', hidden: true },
  { name: 'name', hidden: true },
  { name: 'purchaseOrderNumber', hidden: true },
];
...
export const TableColumns = [
  ...
  },
  {
    ...
    align: 'right',
    width: '120px',
    render: (it) => Number(it.unitCost).toFixed(2),
  },
  {
    header: 'Qty',
    key: 'quantity',
  ...
  },
  {
    ...
    align: 'right',
    width: '80px',
    render: (it) => Number(it.discount).toFixed(2),
  },
  {
    header: 'VAT',
    key: 'vat',
  ...
  },
  {
    ...
    align: 'right',
    width: '140px',
    render: (it) => Number(it.totalAmount || it.amount || 0).toFixed(2),
  },
];

export const ItemsFields = (materials, inv) => [
