...
const initialValues = useMemo(() => ({
  ...
  const fields = [
    ...
    { name: 'email', label: 'Email', type: 'email', span: 'span1', validator: Yup.string().email('Invalid email').required('Email is required') },
    { name: 'spacer-3', type: 'spacer', span: 'span2' },
    { name: 'notedBy', label: 'Noted By', span: 'span1', hidden: true },

    { name: 'contactNumber', label: 'Contact Number', type: 'tel', span: 'span1', validator: Yup.string().required('Contact number is required').matches(/^\+?[0-9\s\-()]{7,}$/, 'Invalid contact number') },
